...
const messages = new Map([
  ...
  [ "party.init20" , "$s{uc:3:Rick} body was found sprawled on the floor of the upstair's bathroom." ], 
  ...
  [ "clue.body1" , "The body has a very hard blow to the back of the head, which bled signicantly--probably cracking the skull." ],
  ...
  [ "clue.body20" , "The body had a keychain with car key, apartment key, mailbox key, office key, and various others." ],
  ...
  [ "clue.house1" , "A search of the rest of the aprtment was generally long and inconclusive, but for one odd fact." ],
  ...
  [ "friday.quot125", "In a seven by seven foot grey-green metal cage in the fifteenth floor of some hundred-year-old penitiary."],
  ...
  [ "bobby.eve45", "I then milled aroung the party for a while." ],
  ...
  [ "fiona.bio85" , "$s{uc:1:Bobby} has been following me around alot, and also hassling Rick." ],

  [ "fiona.bio90" , "I went with Rick to the office party, which turned out to be a real disaster." ],
  [ "fiona.bio95" , "Bobby showed up at the party and started to harrass me." ],
  ...
  [ "fiona.bio140" , "When I headed upstairs for the movie, I was surprised to see only Bobby inside and noone else." ],
  ...
  [ "harold.bio85" , "My wife and I went to the office pary at Rick's place." ],
  ...
  [ "louis.bio5" , "Dom Guiseppe is my cousing and, yes, he's with the local Mafia." ],
  ...
  [ "louis.bio70" , "Unfortuntely, $s{1:Tim} keeps losing on these get-rich-quick schemes, and is continually late on his payments." ],
  ...
  [ "marge.bio15" , "Apparently $s{3:Hal} research hasn't been going well, and he is worrying alot about his tenure, which is up for review." ],
  ...
  [ "marge.bio25" , "I have treid to get him to enjoy himself more, but usually that just leads to arguments." ],
  [ "marge.bio30" , "I first met Rick in Hal's office about 8 months ago.  He actually askedme out on a date not knowing who I was!" ],
  [ "marge.bio35" , "I knew Rick was one of Hal's grad students, but that didn't matter to me." ],
  [ "marge.bio40" , "Perhaps it was just a matter of timining, but I actually took Rick's offer and that was the beginning of our affair." ],
  ...
  [ "marge.bio85" , "While Rick was busy, I ran into Hal and he dragged me into the hall, where he started his typical comnplaining." ],
  ...
  [ "marge.eve35" , "Our argument didn't lastlong, and Hal quickly began to apologize when Tim interrupted as he rushed out of the stairwell." ],
  [ "marge.eve40" , "Afterward, around 8:15, Hal headed outside for a smoke while I went back to the party to mingle for a while." ],
  [ "marge.eve45" , "I found Tim and Rick talkling and Rick invited you up later for the movie, but you didn't talk about anything else." ],
  ...
  [ "marge.others15" , "Fiona is Rickk's old girlfriend whom it seems he has returned to.  I had heard of her, but never met her until tonight." ],
  [ "marge.others20" , "Lousie seems to be a friend of Rick's. I believe he mentioned him before in passing." ],
  ...
  [ "pauline.bio40" , "$s{uc:1:Rick} borrowed alot of cash from some Italian friend in the B-school." ],
  ...
  [ "pauline.eve40" , "I pardened myself to go to the bathroom while Rick continued on to the theater room." ],
  ...
  [ "pauline.others10" , "$s{uc:1:Rick} told me Fiona was an ex-girlfried, but they were getting back together." ],
  ...
  [ "pauline.arg" , "I imagine his new girlfriend couldn've been jealous.  Jealous enough to kill him?  I don't know." ],  
  ...
  [ "tim.bio80" , "I tried to subtlely avoid $s{2:Louis}, but soon he walked by and slipped me this note.  It said, 'Pay up or die!'" ],
...
class MessageMap{
    ...
    static replaceSynonyms( message){
        ...
        while(mMsg.hasMark && mMsg.hasKey()){
            val = SynonymMap.getSynonym(mMsg.key); // get a synonym
    ...
    // called internally only
    static replaceProperties( message, obj, sMark ){
        let val = null;
        let mMsg = new MarkedMessage(message, 0, sMark);
        while(mMsg.hasMark && mMsg.hasKey()){
            val = obj[mMsg.key] ?? null; // get the property
    ...
    static getMessage(msgKey, gameState){
        let message = String(messages.get(msgKey) ?? msgKey);
        MessageMap.log("processing synonyms...");
        message = MessageMap.replaceSynonyms(message);
        if(gameState.player){
            MessageMap.log("processing player: " + message);
            message = MessageMap.replaceProperties(message, gameState.player , pS );
        }
        if(gameState.location){
            MessageMap.log("processing location: " + message);
            message = MessageMap.replaceProperties(message, gameState.location , lS );
        }
        return message;
    }
