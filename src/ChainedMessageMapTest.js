...
class ChainedMessageMapTest extends Component{
  ...
  componentDidMount(){
    this.keys = ChainedMessageMap.getChainedMessageKeys();
    // console.log("ChainedMessageMap keys:" + this.keys);
  ...
  tick() {
      if(this.loopCount < this.loopMax){
        ...
        let value = ChainedMessageMap.getChainedMessages( key, this.gameState );
